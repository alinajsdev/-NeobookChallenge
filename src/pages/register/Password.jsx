...
const Password = ({ username, email }) => {
  ...
  <Flex flexDirection={'column'} >
    ...
    <Heading mt={'28px'} mb={'5px'} color={'#494949'} fontSize={'20px'} fontFamily={'Inter'} fontWeight={'500'}>Придумайте пароль</Heading>
    <Text color={'#C0C0C0'} fontSize={'16px'} fontFamily={'Inter'} fontWeight={'400'} w={'343px'} textAlign={'center'} mb={'28px'}>Минимальная длина — 8 символов. Для надежности пароль должен содержать буквы и цифры.</Text>
    </Flex>
    <form onSubmit={handleSubmit} style={{position:"relative"}}>

<Box>
<FormLabel style={{ opacity: values.password ? 1 : 0 }}    color={"#C0C0C0"}
          fontSize={"14px"}
          fontFamily={"Inter "}
          mt={"0px"}
          position={"absolute"}
          top={"-6px"}
          fontWeight={"400"}>Пароль</FormLabel>
  <Input
...
<FormLabel  style={{ opacity: values.confirm_password ? 1 : 0 }}  color={"#C0C0C0"}
          fontSize={"14px"}
          fontFamily={"Inter "}
       
          position={"absolute"}
          top={"-6px"}
          fontWeight={"400"}>Повторите пароль</FormLabel>
  <Input
    onChange={handleChange}
    onBlur={handleBlur}
